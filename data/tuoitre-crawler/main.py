...
def crawl(file_path, category, limit, delay, crawl_comment, newer_only, extend):
  crawled_ids = set()
  write_mode = "w"
  ...
  crawler = TuoiTreCrawler(category=category, delay=delay, crawl_comment=crawl_comment, skip_these=crawled_ids, newer_only=newer_only)
...
if __name__ == "__main__":
  ...
  group.add_argument("-c", "--crawl", help="crawl the TuoiTre website", action="store_true")
  parser.add_argument("-n", "--number", help="number of articles to crawl", type=int)
  parser.add_argument("--no-comment", help="do not crawl the comments of the articles", action="store_true")
  parser.add_argument("-d", "--delay", help="delay between requests", type=float, default=1)
  parser.add_argument("--category", help="category of the articles to crawl", type=str, default="moi-nhat")
  parser.add_argument("--newer-only", help="only crawl articles newer than the last crawled article", action="store_true")
  parser.add_argument("-e", "--extend", help="extend the crawled articles", action="store_true")
  
  parser.add_argument("-s", "--sort", help="sort the crawled articles", action="store_true")
  
  parser.add_argument("-p", "--preview", help="print a preview of the crawled articles", action="store_true")

  parser.add_argument("-t", "--test", help="test duplication in the dataset", action="store_true")
  ...
  if args.crawl:
    ...
    if args.file is None:
      parser.error("Please specify the file to store the crawled data with --file")
